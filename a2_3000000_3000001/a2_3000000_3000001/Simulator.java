...
public class Simulator {
	...
	public void simulate() {
		...
		Spot[] currentlyParkedSpots = new Spot[60]; 
		Spot firstCarInQueue = null;
		Spot firstCarOutQueue = null;
		RandomGenerator generators = new RandomGenerator();
		...
		while (clock < steps) {
			...
			if (generators.eventOccurred(probabilityOfArrivalPerSec) == true){
				Spot newCarSpot = new Spot(generators.generateRandomCar(), 0);
			...
			for (int i=0; i<lot.getNumRows(); i++){
				for (int j=0; j<lot.getNumSpotsPerRow(); j++){
					if (lot.getSpotAt(i,j) != null){
						parkDuration = this.clock - lot.getSpotAt(i,j).getTimestamp();
						if (parkDuration == MAX_PARKING_DURATION){
							outgoingQueue.enqueue(lot.getSpotAt(i,j));
							lot.remove(i, j);
						} else {
							if (generators.eventOccurred(departurePDF.pdf(parkDuration)) == true){
								outgoingQueue.enqueue(lot.getSpotAt(i,j));
								lot.remove(i, j);
							}
						}
			...
			if (!incomingQueue.isEmpty() || firstCarInQueue != null){
				if (firstCarInQueue == null){
					firstCarInQueue = incomingQueue.dequeue();
				}
				firstCarInQueue.setTimestamp(clock);
				if (lot.attemptParking(firstCarInQueue.getCar(), firstCarInQueue.getTimestamp()) == true){
					System.out.println(firstCarInQueue.getCar() + " ENTERED at timestep " + clock + "; occupancy is at " +
					lot.getTotalOccupancy());

					firstCarInQueue = null;
				}
			}
			
			if (!outgoingQueue.isEmpty()){
				firstCarOutQueue = outgoingQueue.dequeue();
				System.out.println(firstCarOutQueue.getCar() + " EXITED at timestep " + clock + "; occupancy is at " +
				lot.getTotalOccupancy());
			}
			clock++;
		}
